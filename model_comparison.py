...
class ModelComparisons:
    ...
    def load_models(self):
        ...
        for model_name, filename in self.model_filenames.items():
            ...
            if not model_load_flags.get(model_name, False):
                print(f"Skipping {model_name} as its loading flag is set to False.")
                continue
            try:
    ...
    def plot_model_error_details(self, model_name):
        ...
        if metric_values[1] is None:
            warnings.warn(f"Median Absolute Error is not available for model '{model_name}'.")
            metric_values[1] = 0

        # Create 2x2 grid plot
        fig, axes = plt.subplots(2, 2, figsize=(16, 6))
        axes = axes.flatten()
        colors = self.model_colors.get(model_name, '#D3D3D3')  # Default: Light Gray
        ...
        for ax, metric, value, title in zip(axes, metrics, metric_values, titles):
            ...
            ax.bar([model_name], [value], color=colors, edgecolor='black', linewidth=0.7)
            ax.set_title(title, fontsize=14)
            ax.set_ylabel("Error")
            ax.yaxis.grid(True, linestyle='--', linewidth=0.5, color='gray', alpha=0.7)

            # Add text label on the bar
            ax.text(0, value + 0.01 * max(metric_values), f'{value:.2f}', ha='center', va='bottom', fontsize=10, color='black')
